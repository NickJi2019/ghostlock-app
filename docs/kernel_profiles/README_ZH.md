...
- `app/src/main/assets/kernel_profiles/defaults.json`：所有内核版本共用的的默认执行参数，主要定义等待时间/重试次数，也可进行自定义，详见 [公共执行默认值](defaults_ZH.md)。
- `app/src/main/assets/kernel_profiles/<uname-r>.json`：每个内核细分版本对应的完整 profile；文件名与 内核细分版本 `release` 必须一致。
...
3. 复制对应 `.template.json`，以完整 内核细分版本 `release` 命名并写入必填字段。
